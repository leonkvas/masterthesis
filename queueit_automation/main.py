...
class Purchase:
    ...
    def __init__(self, url, user_agent=None, proxy=None):
        ...
        self.Session = SessionWrapper(proxy=initial_proxy)
    ...
    def load_proxies(self):
        ...
        try:
            ...
            if os.path.exists(proxy_file):
                ...
                if self.ProxyList:
                    print(f"[PROXY] Loaded {len(self.ProxyList)} proxies from proxies.txt")
                else:
                    print("[PROXY] No proxies found in proxies.txt")
            else:
                print("[PROXY] proxies.txt file not found. Running without proxies.")
        except Exception as e:
            print(f"[PROXY] Error loading proxies: {str(e)}")
    ...
    def error_with_ip_change(self, message):
        ...
        print(f"[ERROR] {message} - IP change needed")
    ...
    def rotate_proxy(self):
        ...
        if new_proxy:
            print(f"[PROXY] Rotating proxy to: {new_proxy}")
            # Create a new session with the new proxy
            self.Session = SessionWrapper(proxy=new_proxy)
            self.Proxy = new_proxy
            # Add sleep after proxy rotation to allow connection to stabilize
            time.sleep(3)
            return True
        else:
            print("[PROXY] No proxies available for rotation")
    ...
    def extension_checkout(self, link, cookies, _, __):
        ...
        print(f"[CHECKOUT] Proceeding to checkout at: {link}")
        print(f"[CHECKOUT] With cookies: {cookies}")
        return None
    
    def track_checkouts(self):
        """Simulate tracking checkouts"""
        print("[SYSTEM] Tracking successful checkout")
    
    def wait_delay_monitor(self):
        """Simulate waiting for next poll"""
        print("[SYSTEM] Waiting for next queue poll...")
        time.sleep(5)
    
    def running_decorator(self, name):
        """Decorator for running methods with logging"""
        def decorator(func):
            def wrapper(*args, **kwargs):
                print(f"[RUN] Starting: {name}")
                result = func(*args, **kwargs)
                print(f"[RUN] Completed: {name}")
                return result
            return wrapper
        return decorator
...
class SessionWrapper:
    ...
    def __init__(self, proxy=None):
        ...
        if proxy:
            try:
                print(f"[PROXY] Setting up proxy: {proxy}")
                ...
                if '@' in proxy:
                    ...
                    if ':' in proxy.split('@')[0]:
                        ...
                        print("[PROXY] Using provided username:password authentication")
                        ...
                        print("[PROXY] Adding default password to proxy authentication")
                else:
                    # No authentication in URL, may need to be added separately
                    # For Panaio proxies, username is typically in format: pkg-packet-country-US
                    # and password is simply "password"
                    print("[PROXY] No authentication in proxy URL, using proxy as is")
                    proxy_formatted = f"http://{proxy}"
                
                print(f"[PROXY] Formatted proxy URL: {proxy_formatted.replace(':password@', ':***@')}")
                ...
                print("[PROXY] Testing proxy connection...")
                test_response = self.session.get("https://httpbin.org/ip", timeout=10)
                if test_response.status_code == 200:
                    print(f"[PROXY] Proxy test successful: {test_response.json()}")
                else:
                    print(f"[PROXY] Proxy test returned status code: {test_response.status_code}")
                    
            except requests.exceptions.ProxyError as pe:
                print(f"[PROXY] Proxy configuration error: {pe}")
                if "407" in str(pe):
                    print("[PROXY] 407 Proxy Authentication Required - Please check your proxy credentials")

                # Don't set proxies if there was an error
                self.session.proxies = {}
                
            except Exception as e:
                print(f"[PROXY] Error configuring proxy: {str(e)}")
                self.session.proxies = {}
    ...
    def post(self, url, headers=None, json=None, data=None, allow_redirects=True):
        ...
        try:
            ...
            if response.status_code == 407:
                print("[ERROR] 407 Proxy Authentication Required - Check your proxy credentials")
                print("[INFO] For Panaio proxies, make sure to use the correct password (usually 'password')")
            ...
            print(f"[ERROR] Proxy error: {pe}")
            print("[INFO] This could be due to incorrect proxy credentials or connectivity issues")
...
def main():
    """
    Main function to demonstrate Queue-it automation
    """
    # Ensure proxies.txt exists
    create_empty_proxies_file()
    
    # Footlocker Queue-it URL
    target_url = "https://footlocker.queue-it.net/?c=footlocker&e=cxcdtest02&q=746321bb-cfba-4c9c-aeb9-bdb7289315e1&t=https%3A%2F%2Fwww.footlocker.com%2Fsearch%3Fquery%3D6H053P85&cid=de-DE"
    
    print(f"Starting Queue-it automation for: {target_url}")
    
    # Create a Purchase instance
    purchase = Purchase(target_url)
    ...
    print("Making initial request and handling any softblocks...")
    success, error = queue_it.buy()
    
    if error:
        print(f"Error in buy process: {error}")
        return
    
    print(f"Successfully connected to Queue-it")
    print(f"Customer ID: {queue_it.Customer_ID}")
    print(f"Event ID: {queue_it.EventId}")
    
    # Check if we're already in a queue or if the queue has been solved
    if queue_it.queueitsolved:
        print("Queue-it has been solved! Proceeding to target site.")
        if queue_it.TargetUrl:
            print(f"Proceeding to: {queue_it.TargetUrl}")
    elif queue_it.QueueID:
        print(f"Queue ID: {queue_it.QueueID}")
        
        # Poll the queue until we're through
        print("Starting to poll the queue...")
        while True:
            success, error = queue_it.poll_queue()
            
            if error:
                print(f"Error polling queue: {error}")
                break
            
            if success:
                print("Successfully passed the queue!")
                if queue_it.TargetUrl:
                    print(f"Proceeding to: {queue_it.TargetUrl}")
                break
        ...
        print("Not in a queue currently. Check the status.")
        
    print("Queue-it automation completed.")
